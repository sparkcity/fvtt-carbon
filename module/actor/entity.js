...
export default class Actor5e extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("carbon2185", "isPolymorphed") || false;
  }
  ...
  prepareDerivedData() {
    ...
    const flags = actorData.flags.carbon2185 || {};
    ...
    if (this.isPolymorphed) {
      const transformOptions = this.getFlag('carbon2185', 'transformOptions');
      const original = game.actors?.get(this.getFlag('carbon2185', 'originalActor'));
      if (original) {
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("carbon2185", "currencyWeight") ) {
    ...
    if ( this.getFlag("carbon2185", "powerfulBuild") ) mod = Math.min(mod * 2, 8);
  ...
  rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("carbon2185", "reliableTalent"));
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    const feats = this.data.flags.carbon2185 || {};
  ...
  async rollDeathSave(options={}) {
    ...
    const rollData = mergeObject(options, {
      ...
      halflingLucky: this.getFlag("carbon2185", "halflingLucky"),
      targetValue: 10,
      messageData: {"flags.carbon2185.roll": {type: "death"}}
    });
  ...
  async rollHitDie(denomination, {dialog=true}={}) {
    ...
    const roll = await damageRoll({
      ...
      messageData: {"flags.carbon2185.roll": {type: "hitDie"}}
    });
    if ( !roll ) return null;
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    o.flags.carbon2185 = o.flags.carbon2185 || {};
    o.flags.carbon2185.transformOptions = {mergeSkills, mergeSaves};
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.carbon2185.originalActor ) d.flags.carbon2185.originalActor = this.id;
    d.flags.carbon2185.isPolymorphed = true;
    ...
    Hooks.callAll('carbon2185.transformActor', this, target, d, {
  ...
  /**
   * @deprecated since carbon2185 0.97
   */
  getSpellDC(ability) {
